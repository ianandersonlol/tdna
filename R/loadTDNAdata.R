...
loadTDNAdata<-function()
{
  ...
  gff<<-fread(system.file("extdata", "Araport11_GFF3_genes_transposons.201606.gff", package = "tdna"))
  colnames(gff)<-c("chr","source","feature","start","stop","blank","strand","blank2","info")
  confirmed<<-fread(system.file("extdata", "sum_SALK_confirmed.txt", package = "tdna"))
  ...
  location<<-fread(system.file("extdata", "T-DNAall.Genes.Araport11.txt", package = "tdna"))
